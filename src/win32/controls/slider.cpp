...
void SliderImpl::rebuild()
{
    ...
    SendMessage(hwnd, SLM_SETDIRECTION, 0, _vertical ? SLS_VERTICAL : SLS_HORIZONTAL);
    SendMessage(hwnd, SLM_SETRANGE, (WPARAM)_range.first, (LPARAM)_range.second);
    SendMessage(hwnd, SLM_SETPOS, (WPARAM)_value, TRUE);
...
bool SliderImpl::vertical() const
{
    return _vertical;
}

void SliderImpl::vertical(bool value)
{
    if (_vertical == value)
        return;
    _vertical = value;
...
int SliderImpl::value() const
{
    return _value;
}

void SliderImpl::value(int value)
{
    if (_value == value)
        return;
    _value = value;
...
std::pair<int, int> SliderImpl::range() const
{
    return _range;
}

void SliderImpl::range(int from, int to)
{
...
void SliderImpl::colors(Color fgColor, Color bgColor)
{
...
Color SliderImpl::barColor() const
{
    return _barColor;
}

void SliderImpl::barColor(Color value)
{
    if (_barColor == value)
        return;
    _barColor = value;
    ...
    if (handle() == nullptr)
        return;
    auto hwnd = reinterpret_cast<HWND>(handle());
    SendMessage(hwnd, SLM_SETBARCOLOR, 0, (LPARAM)_barColor);
}
...
IEvent<ISharedControl>& SliderImpl::ChangedEvent()
{
