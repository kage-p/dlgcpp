...
void SliderImpl::owner(ISlider* slider)
{
    ControlImpl::owner(slider);

    _slider = slider;

    _slider->colors().event() += [&](auto) { onColorsChanged(); };
    _slider->vertical().event() += [&](auto) { onVerticalChanged(); };
    _slider->barColor().event() += [&](auto) { onBarColorChanged(); };
    _slider->range().event() += [&](auto) { onRangeChanged(); };
    _slider->value().event() += [&](auto) { onValueChanged(); };
    _slider->text().event() += [&](auto) { setText(_slider->text()); };
}

void SliderImpl::build()
{
    ControlImpl::build();
    ...
    setText(_slider->text());

    auto range = _slider->range().value();
    auto value = _slider->value().value();

    auto hwnd = this->hwnd();
    SendMessage(hwnd, SLM_SETDIRECTION, 0, _slider->vertical() ? SLS_VERTICAL : SLS_HORIZONTAL);
    SendMessage(hwnd, SLM_SETRANGE, (WPARAM)range.first, (LPARAM)range.second);
    SendMessage(hwnd, SLM_SETPOS, (WPARAM)value, TRUE);

    updateDisplayStyles();
}

std::string SliderImpl::className() const
{
    return SliderControlClass;
}

void SliderImpl::notify(DialogMessage& msg)
{
    if (msg.wMsg == WM_HSCROLL || msg.wMsg == WM_VSCROLL)
    {
        _slider->value() = (int)msg.wParam;
        _slider->ChangedEvent().invoke();
    }
}

void SliderImpl::onVerticalChanged()
{
    if (hwnd() == 0)
        return;
...
void SliderImpl::onRangeChanged()
{
    if (hwnd() == 0)
        return;
...
void SliderImpl::onValueChanged()
{
    if (hwnd() == 0)
        return;

    SendMessage(hwnd(), SLM_SETPOS, (WPARAM)_slider->value().value(), TRUE);
}

void SliderImpl::onColorsChanged()
{
    if (hwnd() == 0)
        return;
...
void SliderImpl::onBarColorChanged()
{
    if (hwnd() == 0)
        return;

    SendMessage(hwnd(), SLM_SETBARCOLOR, 0, (LPARAM)_slider->barColor().value());
}

void SliderImpl::updateDisplayStyles()
{
    auto clrPair = _slider->colors().value();
    ...
    COLORREF barColor =
        _slider->barColor() != Color::Default
        ? (COLORREF)_slider->barColor().value()
...
typedef struct dlgcpp::controls::TSLDR_DATA {  // Custom data for the control
...
void SliderImpl::sldRedraw(TSLDR_DATA* psd)
{
    RedrawWindow(psd->hwnd, NULL, 0, RDW_ERASE | RDW_INVALIDATE);
}

void SliderImpl::sldMoveIncrement(TSLDR_DATA* psd, bool redraw)
{
    // bounds check
...
void SliderImpl::sldMoveWithMouse(TSLDR_DATA* psd)
{
    auto rc = RECT();
    auto pt = POINT();
...
//------------------------------------------------------------------------------
LRESULT WINAPI SliderImpl::SliderWndProc(HWND hwnd, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    switch (wMsg)
    {
    case WM_CREATE:
